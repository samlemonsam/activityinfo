        ...
        switch(direction) {
            case ASC:
                if (range == null || range.length == numRows) {
                    HeapsortColumn.heapsortBitSet(bitSet, sortVector, numRows, true);
                } else {
                    HeapsortColumn.heapsortBitSet(bitSet, sortVector, range.length, range, true);
                }
                break;
            case DESC:
                if (range == null || range.length == numRows) {
                    HeapsortColumn.heapsortBitSet(bitSet, sortVector, numRows, false);
                } else {
                    HeapsortColumn.heapsortBitSet(bitSet, sortVector, range.length, range, false);
                }
                break;
        }
        return sortVector;
