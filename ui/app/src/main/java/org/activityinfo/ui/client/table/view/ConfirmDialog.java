...
/**
 * Generic user interface for the user to confirm an action which will have strong effects
 * and may fail.
 *
 */
public class ConfirmDialog {
    ...
    private ConfirmDialog() {
        ...
        container.add(failedMessageContainer);
        container.add(messageContainer);

        dialog = new Dialog();
        dialog.setModal(true);
        dialog.setPixelSize(300, 400);
        dialog.setWidget(container);
        dialog.setPredefinedButtons(Dialog.PredefinedButton.OK, Dialog.PredefinedButton.CANCEL);
        dialog.getButton(Dialog.PredefinedButton.OK).addSelectHandler(this::tryAction);
    ...
    private void tryAction(SelectEvent event) {
    ...
    private void updateState(State state, Throwable caught) {
        ...
        dialog.getButton(Dialog.PredefinedButton.OK).setEnabled(state != State.PROGRESS);
        dialog.getButton(Dialog.PredefinedButton.CANCEL).setEnabled(state != State.PROGRESS);
        ...
        switch (state) {
            ...
            case FAILED:
                ...
                failedMessageContainer.setHTML(I18N.CONSTANTS.serverError());
                break;
    ...
    private void updateMessages(Messages messages) {
        dialog.setHeading(messages.getTitleText());
        messageContainer.setHTML(messages.getMessageText());

        if(state == State.PROGRESS) {
            dialog.getButton(Dialog.PredefinedButton.OK).setText(messages.getPrimaryButtonText());
        } else {
            dialog.getButton(Dialog.PredefinedButton.OK).setText(messages.getPrimaryButtonText());
        }
    }
