...
public class BillingWarning extends Composite {
    ...
    public BillingWarning(AccountStatus status) {
        ...
        if(status.isTrial()) {
            if(status.isExpired()) {
                ...
                warningText.setInnerHTML("<strong>Your Trial Subscription is " + status.expiringIn(new Date()) + ".</strong> " +
            ...
            warningText.setInnerHTML("<strong>Your Subscription is " + status.expiringIn(new Date()) + ".</strong> " +
    ...
    public void closeButtonClick(ClickEvent event) {
        BillingSupervisor.snooze(status, BillingSupervisor.BILLING_SNOOZE_KEY_PREFIX);
