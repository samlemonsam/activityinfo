 ...
 */
public class EnumItemSuggestOracle extends SuggestOracle {

    private List<EnumItem> enumItems;
    private LatinPlaceNameScorer scorer = new LatinPlaceNameScorer();

    public EnumItemSuggestOracle(List<EnumItem> enumItems) {
    ...
    public void requestSuggestions(Request request, Callback callback) {
        ...
        for (EnumItem enumItem : enumItems) {
            if (scorer.score(request.getQuery(), enumItem.getLabel()) > 0.5) {
